...
const sanitizeEmail = (raw?: string | null) => {
  const e = (raw ?? "").trim();
  return !e || looksLikePlaceholder(e) ? "" : e;
};
...
function buildCustomerUrl(offerId: string, offerNumber: string) {
  ...
  try {
    ...
    // Fallback utan token – adminmejlet ska inte falla pga länkbygget
    return `${base}/offert/${encodeURIComponent(offerNumber)}?view=inkommen`;
  }
}

const adminStartUrl = () => `${ADMIN_BASE.replace(/\/+$/, "")}/start`;
...
function tripBlockHtml(p: SendOfferParams) {
  ...
  const rows = [
    ...
    row("Datum", p.date),
    row("Tid", p.time),
    row("Passagerare", (p.passengers ?? "—").toString()),
    p.via ? row("Via", p.via) : "",
    p.onboardContact ? row("Kontakt ombord", p.onboardContact) : "",
  ].join("");
  const ret = (p.return_from || p.return_to || p.return_date || p.return_time)
    ? `<div style="font-weight:600;margin:14px 0 6px">Retur</div>${
        row("Från", p.return_from)+row("Till", p.return_to)+row("Datum", p.return_date)+row("Tid", p.return_time)
      }`
    : "";
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse"><tbody>${rows}${ret}</tbody></table>`;
}
  ...
  <div style="background:#f7f7f8;padding:24px 0">
    <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="width:100%;max-width:640px;border-collapse:collapse">
      <tr><td style="padding:0 16px">
        ...
        <div style="color:${BRAND.muted};font-size:12px;margin-top:12px;text-align:left">${BRAND.name} • Detta är ett automatiskt meddelande – svara gärna om du vill komplettera något.</div>
      </td></tr>
    </table>
  </div>`;
}
...
function renderAdminHtml(p: SendOfferParams) {
  const inner = `
    ...
    ${tripBlockHtml(p)}
    ${p.notes ? `<hr style="border:none;border-top:1px solid ${BRAND.border};margin:16px 0"/><div style="font-weight:600;margin-bottom:6px">Övrigt</div><div>${safe(p.notes)}</div>` : ""}
  `;
  return renderWrapper(inner, "Ny offertförfrågan", undefined, { href: adminStartUrl(), label: "Öppna i portalen" });
}
...
function renderCustomerHtml(p: SendOfferParams) {
  ...
  return renderWrapper(inner, "Vi har mottagit din offertförfrågan", undefined, { href, label: "Visa din offert" });
}
...
function renderText(p: SendOfferParams) {
  ...
  if (p.return_from || p.return_to || p.return_date || p.return_time) {
    lines.push("", "Retur", `Från: ${p.return_from || "-"}`, `Till: ${p.return_to || "-"}`, `Datum: ${p.return_date || "-"}`, `Tid: ${p.return_time || "-"}`);
  }
...
async function sendViaResend(args: { to: string; subject: string; html: string; text: string; bcc?: string[] }) {
  ...
  return { ok: true, id: r?.data?.id || null, from, to: args.to };
}

export async function sendOfferMail(p: SendOfferParams) {
  const cleanCustomerEmail = sanitizeEmail(p.customerEmail);
  const pClean: SendOfferParams = { ...p, customerEmail: cleanCustomerEmail };
  ...
  const result: any = { admin: null, customer: null };

  // 1) Admin först – får ALDRIG falla pga kundlänk/token
  if (adminTo) {
    try {
      result.admin = await sendViaResend({ to: adminTo, subject: adminSubject, html: htmlAdmin, text });
    } catch (e: any) {
      result.admin = { ok: false, error: e?.message || String(e) };
    }
  } else {
    result.admin = { ok: false, error: "OFFERS_INBOX/ADMIN saknas" };
  }
  ...
  if (isValidEmail(toCustomer)) {
    try {
      ...
      result.customer = await sendViaResend({ to: toCustomer, subject: customerSubject, html: htmlCustomer, text, bcc: bccList });
    } catch (e: any) {
      result.customer = { ok: false, error: e?.message || String(e) };
    }
  } else {
    result.customer = { ok: false, error: `Ogiltig kundadress: "${toCustomer}"` };
  }
