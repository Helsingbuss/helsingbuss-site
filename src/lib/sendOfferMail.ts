...
async function renderText(p: SendOfferParams): Promise<string> {
  const link = await buildSecureOfferLink(p.offerId);
  const lines = [
 ...
 * Låter dig anropa med (to, offer_number, status) om du har gammal kod.
 * Den skapar ett minimalt SendOfferParams-objekt och kallar sendOfferMail().
 */
export async function sendOfferMailSimple(
  to: string,
  offer_number: string,
  status: string
) {
  // Minimal fake-ID för länk – byt gärna till riktigt ID om du har det tillgängligt.
  const pseudoId = offer_number;

  return await sendOfferMail({
    offerId: pseudoId,
    offerNumber: offer_number,
    customerEmail: to,
    // Övriga fält kan fyllas på här vid behov
  });
}
