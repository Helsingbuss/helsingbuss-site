...
function LinesEditor({
  ...
  function updLineTitle(idx: number, title: string) {
    const next = [...lines];
    next[idx] = { ...next[idx], title };
    setLines(next);
  }
  function addStop(idx: number) {
    const next = [...lines];
    next[idx] = {
      ...next[idx],
  ...
  function updStop(idx: number, sIdx: number, patch: Partial<LineStop>) {
    const next = [...lines];
    const row = next[idx].stops[sIdx] || { name: "", time: "" };
    next[idx].stops[sIdx] = { ...row, ...patch };
    setLines(next);
  }
          ...
          <div className="mt-3">
            ...
            <div className="mt-2 space-y-2">
              {ln.stops.map((st, sIdx) => (
                <div
                  key={sIdx}
                  className="grid md:grid-cols-[1fr_160px_auto] gap-2"
                >
                  <input
                    className="border rounded-xl px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-[#194C66]/30"
...
export default function NewTripPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!r.ok) throw new Error("Kunde inte ladda operatörer/bussar.");
        const j = await r.json();
        if (!j.ok) throw new Error(j.error || "Tekniskt fel.");
        ...
        if (!cancelled) {
          console.error(e);
          setFleetErr(e?.message || "Kunde inte ladda fordonsdata.");
        }
      }
    })();
  ...
  async function onSave() {
    ...
    try {
      ...
      const badge = badgeText.trim()
        ? [
            badgeText.trim(),
            /^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(badgeBg.trim())
              ? badgeBg.trim()
              : "",
            /^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(badgeFg.trim())
              ? badgeFg.trim()
              : "",
          ]
            .filter(Boolean)
  ...
  const previewDatesText = useMemo(() => {
    if (f.departures_coming_soon) {
      return "Avgångsorter och datum kommer inom kort.";
    }
    if (uniqueDates.length === 0) return "";
    if (uniqueDates.length === 1)
      return `Nästa avgång: ${fmtDate(uniqueDates[0])}`;
    const shown = uniqueDates.slice(0, 3).map(fmtDate).join(", ");
    const rest = uniqueDates.length - 3;
    return rest > 0
      ? `Flera datum: ${shown} + ${rest} till`
      : `Flera datum: ${shown}`;
  }, [uniqueDates, f.departures_coming_soon]);
  ...
  const previewBadgeSpec = useMemo(() => {
    const ui = badgeText.trim()
      ? [badgeText.trim(), badgeBg.trim(), badgeFg.trim()]
          .filter(Boolean)
          .join("|")
      : f.badge;
    ...
    () => (
      <div className="rounded-2xl overflow-hidden border bg-white shadow-sm">
        ...
        <div className="relative bg-[#f3f4f6] aspect-[600/390]">
          ...
          {previewBadgeSpec && (
            <div
              ...
              style={{
                background: previewBadgeSpec.bg,
                color: previewBadgeSpec.fg,
              }}
            >
        ...
        <div className="p-4">
          ...
          {f.external_url && (
            <div className="mt-2 text-xs text-[#0f172a]/60 truncate">
              Länk: {f.external_url}
            </div>
          )}
        </div>
        ...
        <main className="px-6 pb-16 pt-14 lg:pt-20">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <Card title="Media">
                <div className="grid md:grid-cols-[1fr_auto] gap-4 items-end">
                  <div>
                    ...
                    />
                    <Help>
                      Alternativ: ladda upp fil här nedan så fylls URL i
                      automatiskt. Rekommenderad storlek: <b>600×390 px</b>
                    </Help>
                  </div>
                  <div>
              ...
              <Card
                title="Turlista / avgångar"
                aside={
                  <span className="text-[12px] text-slate-500">
                    Visas i förhandsvisningen
                  </span>
                }
              >
                <div className="mb-3 flex items-center gap-2">
                  <input
                    id="departuresSoon"
                    type="checkbox"
                    checked={!!f.departures_coming_soon}
                    onChange={(e) =>
                      upd("departures_coming_soon", e.target.checked)
                    }
                  />
                  <label
                    htmlFor="departuresSoon"
                    className="text-sm text-[#194C66]/80"
                  >
                    Visa texten{" "}
                    <span className="font-semibold">
                      "Avgångsorter och datum kommer inom kort"
                    </span>{" "}
                    istället för lista med datum.
                  </label>
                </div>
            ...
            <div className="lg:col-span-1">
              <div className="lg:sticky lg:top-16">
                <div className="text-sm text-[#194C66]/70 mb-2">
                  Förhandsvisning
                </div>
                {preview}
              </div>
