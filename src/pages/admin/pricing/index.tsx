...
export default function PricingPage() {
  ...
  // Datumlista för vald resa:
  // 1) försök från trip_departures
  // 2) om inga hittas → fall back till trips.departures (JSONB)
  const departuresForTrip = useMemo(() => {
    if (!selectedTripId) return [];

    const unique = new Set<string>();

    // 1) från tabellen trip_departures
    departures
      .filter((d) => d.trip_id === selectedTripId)
      .forEach((d) => {
        const raw = d.depart_date ? String(d.depart_date) : "";
    ...
    // 2) fallback till trips.departures (JSON) om tabellen är tom
    if (unique.size === 0) {
      const t = trips.find((x) => x.id === selectedTripId);
      if (t && t.departures) {
        let rawList: RawDeparture[] = [];

        if (Array.isArray(t.departures)) {
          rawList = t.departures as RawDeparture[];
        } else if (typeof t.departures === "string") {
          try {
            const parsed = JSON.parse(t.departures);
            if (Array.isArray(parsed)) rawList = parsed as RawDeparture[];
          } catch {
            // ogiltig JSON – skippa
          }
        }
        ...
        for (const r of rawList) {
          const rawDate =
            (r.dep_date ||
              ...
              r.when ||
              "") as string;
          const dd = String(rawDate).slice(0, 10);
          if (!dd) continue;
          if (/^\d{4}-\d{2}-\d{2}$/.test(dd)) {
            unique.add(dd);
          }
    ...
    out.sort();
    return out;
  }, [departures, trips, selectedTripId]);
                ...
                <div className="px-5 pt-4 pb-5 space-y-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      ...
                      <div className="text-[11px] text-slate-500 mt-1">
                        Datum hämtas från{" "}
                        <code>trip_departures</code> (eller{" "}
                        <code>trips.departures</code> som fallback).
                      </div>
                    </div>
